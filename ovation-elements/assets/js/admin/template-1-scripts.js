...
jQuery(document).ready(function($) {
    function updateLivePreview() {
      ...
      // Slide Image Preview
         ...
         var slideBgColor = $('#slide_bg_color_0').val(); // Get the selected background color
         var swiperWrapper = $('.swiper-wrapper.swiper-wrapper-business'); // Select the swiper-wrapper element
         ...
         if (slideImageID) {
             ...
             attachment.fetch().then(function() {
                 if (attachment && attachment.get('url')) {
                    ...
                    //  swiperWrapper.css('background', 'url(' + imageUrl + ') no-repeat center center');

                    //  swiperWrapper.css('background-size', 'cover'); 

                     swiperWrapper.css({
                        'background': 'url(' + imageUrl + ') no-repeat center center',
                        'background-size': 'cover',
                        'background-color': slideBgColor // Set the background color from the color picker
                    });
...
jQuery(document).ready(function($){
    ...
    // $(document).on('click', '.remove_bg_image_button', function (e) {
    //     e.preventDefault();
    
    //     const button = $(this);
    //     const slideContainer = button.closest('.slide-container');
    
    //     // Clear the hidden input value (image ID) and hide the image
    //     slideContainer.find('input[type="hidden"]').val('');
    //     slideContainer.find('img').hide(); // Hides the image preview
